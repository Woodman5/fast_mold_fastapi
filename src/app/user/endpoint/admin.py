...
@admin_router.get('/{pk}', response_model=schemas.User)
...
async def create_user(schema: schemas.UserCreate, user: models.UserModel = Depends(get_superuser)):
    """ Create user """
    # TODO исправить создание юзера, параметры: is_active & is_superuser
    return await service.user_s.create_user(schema)


@admin_router.put('/{pk}', response_model=schemas.User)
